...
function nearBestPosition(whereId, theObjId, centerOnEl) {
  ...
  if (el) {
    ...
    var left = trueX;
    var top = trueY;
    var barHeight = (!$.support.cssFloat) ? 45 : 35;
    var barWidth = (!$.support.cssFloat) ? 20 : 0;
...
jQuery.fn.createErrorAlert = function(errorCode, message) {
  this.each(function() {
    ...
    if ($("#" + el.attr("id") + "error").size() <= 0) {
      ...
      err=$(err);
      err.attr("title",errMess).attr("src",CtxVar.path+"/static/js/thirdplugin/jQueryGantt/alert.gif");
      el.after(err);
    }
  });
  return this;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  bdiv.append("<img src=\""+CtxVar.path+"/static/js/thirdplugin/jQueryGantt/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url("+CtxVar.path+"'/static/js/thirdplugin/jQueryGantt/res/img/black_70.png')"});
  ...
  var img=$("<img src='"+CtxVar.path+"/static/js/thirdplugin/jQueryGantt/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
